...
export function renderPreviewVerifyAgentTaskBlock(args: {
  ...
  const lines = [
    ...
    "    ────────────────────────────────",
    "    NEXT ON-DEVICE — the last check happens on the Ledger screen:",
    "      • If the device BLIND-SIGNS (shows only a hash — typical for swaps",
    "        and most DeFi calls): check that the hash on your Ledger screen",
    "        is exactly the same as:",
    "",
    `            \`${args.preSignHash}\``,
    "",
    "        If it differs in any character → REJECT on-device.",
    "      • If the device CLEAR-SIGNS (shows decoded fields — Aave / Lido /",
    "        1inch / LiFi / approve plugins): hash matching does NOT apply.",
    "        Check that the function name + key fields (amount, recipient,",
    "        spender) shown on your Ledger screen match the compact summary",
    "        above. Anything different → REJECT on-device.",
    "    ════════════════════════════════",
    "",
    ...
    "",
    "Render the blind-sign hash on its own line, wrapped in single backticks",
    "exactly as shown in the template above (`0x…`), so Markdown-rendering",
    "chat clients display it as highlighted inline code rather than blending",
    "into prose. Do NOT strip the backticks and do NOT inline the hash into",
    "the sentence — the user must be able to spot it at a glance.",
    "",

...
export function renderAgentTaskBlock(
  ...
  const lines = [
    ...
    `     from this same prepared-tx response>" }. The server runs the same`,
    `     algorithm swiss-knife uses in-browser (fetch the function signature`,
    `     from 4byte.directory, decode via viem, re-encode to prove bytes`,
    `     round-trip losslessly) but server-side, so the user sees the result`,
    `     in chat without opening a browser. Do NOT script your own WebFetch`,
    `     to 4byte or swiss-knife; always call this tool so the check runs`,
    ...
    `  4. After the verification block, OFFER the user a further check in a`,
    ...
    `     compromised server could in principle lie in both. Say in one`,
    `     short sentence that the user can EITHER open the swiss-knife URL`,
    `     in their own browser (runs the same algorithm in a context they`,
    `     control), OR ask you to decode the calldata yourself right here`,
    `     using your built-in ABI knowledge (model weights are a different`,
    `     trust boundary from server code — any disagreement is a real`,
    `     signal). Phrase both as options the user can pick; do NOT perform`,
    `     your own decode unless they ask.`,
    `  5. End your reply with a one-line reminder: the send_transaction call`,

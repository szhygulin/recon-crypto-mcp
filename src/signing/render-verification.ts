...
export function renderPreviewVerifyAgentTaskBlock(args: {
  ...
  const lines = [
    ...
    `        and most DeFi calls): match it against ${args.preSignHash}`,
    "        from the LEDGER BLIND-SIGN HASH block above. Mismatch → REJECT",
    "        on-device.",
    "      • If the device CLEAR-SIGNS (shows decoded fields — Aave / Lido /",
    "        1inch / LiFi / approve plugins): hash matching does NOT apply.",
    "        Verify the function name + key field (amount, recipient, spender)",
    "        on-device match the compact summary above. Anything different",
    "        → REJECT on-device.",
    "    ════════════════════════════════",
    "",

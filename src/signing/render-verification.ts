...
export function renderPreviewVerifyAgentTaskBlock(args: {
  ...
  const lines = [
    ...
    "",
    "FOURTH OPTION — independent hash recompute. Wording suggestion:",
    ...
    "",
    "Skip the FOURTH-option offer (hash recompute) entirely for native sends",
    "where the agent already knows the full tuple (data='0x') and `to`/`value`",
    "eyeballing covers the check. The FIFTH option below still applies.",
    "",

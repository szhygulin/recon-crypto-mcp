...
export function renderPreviewVerifyAgentTaskBlock(args: {
  ...
  const lines = [
    ...
    "bytes Y).",
    "",
    "Offer BOTH of the following to the user as trust-boundary options (the",
    "first three were at prepare time). Present them together so the user",
    "can pick either or both. Do NOT run either unprompted.",
    "",
    "FOURTH OPTION — (tuple, hash) pair-consistency check. NARROWER than (b)",
    "at prepare time: (b) catches malicious CALLDATA; this catches a pinned",
    ...
    "",
    "Skip the FOURTH-option offer (pair-consistency) entirely for native",
    "sends where the agent already knows the full tuple (data='0x') and",
    "`to`/`value` eyeballing covers the check. The FIFTH option below still",
    "applies.",
    "",
    "FIFTH OPTION — second-agent verification (strongest of the four; catches",
    "a fully-coordinated compromise where THIS agent and the MCP could be",
    "lying together). Wording suggestion:",
    ...
    "and relay the artifact JSON + its `instructionsForSecondAgent` field",
    "VERBATIM so the user can copy-paste them into the second session. Do",
    "NOT pre-decode the bytes yourself in the same reply — the whole point",
    "is that the second agent decodes from scratch without seeing your",
    "description. Remind the user to compare the second agent's plain-",
    "English description to what they asked for, and to match",
    `artifact.preSignHash against the Ledger screen before approving.`,
  ];
  return lines.join("\n");
}

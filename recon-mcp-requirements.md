...
- Add non-EVM chains:
  ...
  - Bitcoin — deferred. A prior iteration shipped read-only balances + consolidation-based send preparation via mempool.space, but was removed pending a real signing story. Ledger Live's WalletConnect integration does not currently support the `bip122:` namespace (confirmed against `LedgerHQ/wallet-connect-live-app`), so there is no hardware-wallet path that works with the existing pairing flow. Reopening this will require either (a) direct USB/HID integration with the Ledger device via `@ledgerhq/hw-app-btc`, or (b) a descriptor-import + external-signing flow (Sparrow, Electrum). Smart-contract-dependent tools (Aave, Uniswap, staking, security) remain N/A for Bitcoin regardless
